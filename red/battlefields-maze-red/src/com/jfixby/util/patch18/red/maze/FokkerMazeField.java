...
class FokkerMazeField implements MazeField {
	...
	public void print () {
		...
		for (int y = 0; y < blocks_heighth; y++) {
			for (int x = 0; x < blocks_width; x++) {
				L.d_appendChars(f[x][y].getSimbol() + "");
			}
			L.d();
		}
	...
	private void expandMazeFrom (XY previous, XY marker, float p_direct, int depth) {
		{
			...
			this.setWall(marker.x, marker.y, resolve(marker.x, marker.y, next.x, next.y),
	...
	public OriginalGameField build () {
		// OriginalGameField
		OriginalGameField of = new OriginalGameField(this.blocks_width, blocks_heighth);
		of.useWall(false);
